...
export const Multivariables = {
	...
	computed: {
		...
		nothingSelectedValues(): Set<string> {
			return this.$store.getters['accessRights/getNothingSelectedValue'](this.section.sectionCode, this.value.id);
		},
		currentAlias(): ?string {
	...
	methods: {
		showSelector(): void {
			const dialog = new Dialog({
				...
				events: {
					...
					onHide: this.setNothingSelectedValueIfNeeded,
				},
				entities: [
					{
		...
		setNothingSelectedValueIfNeeded(): void {
			...
			{
				for (const nothing of this.nothingSelectedValues)
				{
