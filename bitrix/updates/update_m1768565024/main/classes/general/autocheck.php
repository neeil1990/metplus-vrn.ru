...
class CAutoCheck
{
	...
	public static function CheckKernel($arParams)
	{
		...
		if (empty($session["BX_CHECKLIST"][$arParams["TEST_ID"]]))
		{
			$session["BX_CHECKLIST"][$arParams["TEST_ID"]] = [];
		}
		$NS = &$session["BX_CHECKLIST"][$arParams["TEST_ID"]];
		...
		if (!$arError && !$skip)
		{
			...
			else
			{
				...
				{
					$NS["MESSAGE"][$moduleId] .= GetMessage("CL_UNKNOWN_FILES", ["#module_id#" => $moduleId]) . "\n" . $state["UNKNOWN_MESSAGE"];
				}
			}
			$NS["FILES_COUNT"] += $fileCount;
			$NS["MOD_FILES_COUNT"] += $modifiedFileCount;
			$NS["UNKNOWN_FILES_COUNT"] += $unknownFileCount;
		}
		...
		if (($NS["MNUM"] + 1) >= $modulesCount)
		{
			$arDetailReport = "";
			foreach ($NS["MESSAGE"] as $moduleMessage)
			{
				$arDetailReport .= "<div class=\"checklist-dot-line\"></div>" . $moduleMessage;
			}
			$arResult = [
				"MESSAGE" => [
					"PREVIEW" => GetMessage("CL_KERNEL_CHECK_FILES") . $NS["FILES_COUNT"] . "<br>" .
						GetMessage("CL_KERNEL_CHECK_MODULE") . $modulesCount . "<br>" .
						GetMessage("CL_KERNEL_CHECK_MODIFIED") . $NS["MOD_FILES_COUNT"] . "<br>" .
						GetMessage("CL_KERNEL_CHECK_UNKNOWN") . $NS["UNKNOWN_FILES_COUNT"] . "<br>" ,
	...
	public static function CheckQueryString($arParams = [])
	{
		...
		$session = Application::getInstance()->getSession();
		if (empty($session["BX_CHECKLIST"]) || $arParams["STEP"] == 0)
		{
			$session["BX_CHECKLIST"] = [
				"LAST_FILE" => "",
				"FOUND" => "",
				"PERCENT" => 0,
			];
			...
			$session["BX_CHECKLIST"]["COUNT"] = count($files);
		}
		...
		foreach ($arPath as $namespace)
		{
			...
			foreach ($files as $file)
			{
				...
				if (!empty($session["BX_CHECKLIST"]["LAST_FILE"]))
				{
					if ($session["BX_CHECKLIST"]["LAST_FILE"] == $file)
					{
						$session["BX_CHECKLIST"]["LAST_FILE"] = "";
					}
					continue;
				}
				...
				if ($content = file_get_contents($file))
				{
					...
					{
						$session["BX_CHECKLIST"]["FOUND"] .= str_replace(["//", "\\\\"], ["/", "\\"], $file) . "\n";
					}
				}

				if (time() - $time >= 20)
				{
					$session["BX_CHECKLIST"]["LAST_FILE"] = $file;
					return [
						"IN_PROGRESS" => "Y",
						"PERCENT" => number_format($arFileNum / $session["BX_CHECKLIST"]["COUNT"] * 100, 2),
					];
				}
			}
		}
		$arResult = ["STATUS" => true];
		if (!empty($session["BX_CHECKLIST"]["FOUND"]))
		{
			$arResult["STATUS"] = false;
			$arResult["MESSAGE"] = [
				"PREVIEW" => GetMessage("CL_KERNEL_CHECK_FILES") . $arFileNum . ".\n" . GetMessage("CL_ERROR_FOUND_SHORT") . "\n",
				"DETAIL" => GetMessage("CL_DIRECT_QUERY_TO_DB") . "\n" . $session["BX_CHECKLIST"]["FOUND"],
			];
		}
		else
		{
